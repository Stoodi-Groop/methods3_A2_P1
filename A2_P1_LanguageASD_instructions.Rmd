...
pacman::p_load(tidyverse, MuMIn, lme4, lmerTest, purrr, growthcurver)
...
str(df) #Checking the class of the variables
head(df)
```
...
cols <- c("ID", "VISIT", "Ethnicity", "Gender", "Diagnosis")
...
#making models 
m0 <- lmer(CHI_MLU ~ Age + Gender + (1|ID), df, REML = F)
m1 <- update(m0, .~. + VISIT)
m2 <- update(m1, .~. + Diagnosis)
m3 <- update(m2, .~. + VISIT:Diagnosis)
m02 <- lmer(CHI_MLU ~ Age + Gender + (1 + as.numeric(VISIT)|ID), df, REML = F)
m12 <- update(m02, .~. + VISIT)
m22 <- update(m12, .~. + Diagnosis)
m32 <- update(m22, .~. + VISIT:Diagnosis)
...
mib <- lmer(CHI_MLU ~ Gender + Age + Diagnosis*VISIT + I(VISIT^2) + (1 + VISIT| ID) , df, REML = F)

m2 <- lmer(CHI_MLU ~ Diagnosis*VISIT + (1 | ID), df, REML = F)


plot(effects:: allEffects(mib))
...
anova(m32, growth)
scores <- AIC(growth, m32)
scores
>>>>>>> 6dca512364cfecc39a0d2ae98f610767f70ea6bb
```
...
m5 <- lmer(MOT_MLU ~ VISIT + Diagnosis + (1 | ID), df, REML = F)
summary(m1)  
m6 <- lmer(MOT_MLU ~ Diagnosis*VISIT + (1 | ID), df, REML = F)
summary(m2)
m7 <- lmer(MOT_MLU ~ VISIT + (1 | ID), df, REML = F)
summary(m3)
m8 <- lmer(MOT_MLU ~ Diagnosis + (1 | ID), df, REML = F)
summary(m4)
